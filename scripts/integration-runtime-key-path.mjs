...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hagiscript-runtime-key-path-"))
...
const runtimeManifestPath = path.join(tempRoot, "runtime-key-path-manifest.yaml")
...
try {
  diagnostics = await runStage("platform diagnostics", async () => {
    const collected = await collectPlatformDiagnostics({ runProcess, repoRoot, tempRoot })
  ...
  await runStage("prepare key-path manifest", async () => {
    ...
    fs.cpSync(path.join(manifestDirectory, "templates"), path.join(tempRoot, "templates"), {
      recursive: true
    })
  ...
  await runStage("runtime install", async () => {
    ...
    assertIncludes(installOutput, "Runtime install complete.", "runtime install output")
    ...
    const componentNames = report.components.map((item) => `${item.name}:${item.status}`)
  ...
  await runStage("npm-sync tool installation", async () => {
    ...
    assertIncludes(npmSyncOutput, "Plan: skills install", "skills install plan")
    assertIncludes(npmSyncOutput, "Plan: openspec install", "openspec install plan")
  ...
  await runStage("pm2 environment contract", async () => {
    ...
    assertIncludes(envOutput, `PM2 binary: ${getFixturePm2Entrypoint(managedRoot)}`, "pm2 env binary")
    assertIncludes(envOutput, `PM2 home: ${getIntegrationPm2Home(tempRoot, "omniroute")}`, "pm2 env home")
    assertIncludes(envOutput, `Runtime data home: ${path.join(managedRoot, "runtime-data", "components", "services", "omniroute")}`, "pm2 env runtime data")
  ...
  await runStage("pm2 managed service lifecycle", async () => {
    ...
    try {
      for (const service of ["omniroute", "code-server"]) {
        ...
        const statusOutput = await waitForManagedPm2Status(service, "online", {
  ...
  if (enableReleasedServerTest) {
    await runStage("released server key-path", async () => {
      ...
      assertIncludes(installOutput, "Runtime install complete.", "server runtime install")
...
async function runCapture(command, args, cwd) {
  ...
  try {
    ...
    log(`Command completed: ${formatCommand(command, args)}${formatOutputSummary(stdout, stderr)}`)
    return stdout
  } catch (error) {
...
async function prepareReleasedServerPayload(options) {
  ...
  if (!releaseResponse.ok) {
    throw new Error(`Failed to read latest released server metadata: HTTP ${releaseResponse.status}`)
  }
  ...
  if (!asset?.browser_download_url || !asset?.name || !release.tag_name) {
    throw new Error(`Latest release in ${releaseRepository} does not expose a ${assetSuffix} asset.`)
  }
...
async function extractZipArchive(archivePath, extractRoot, cwd) {
  if (process.platform === "win32") {
    await runProcess(
      ...
      { cwd, stdout: "pipe", stderr: "pipe", timeoutMs: runtimeCommandTimeoutMs }
    )
    return
  }
...
function buildGitHubRequestHeaders(accept = "application/octet-stream") {
  ...
  const token = process.env.GITHUB_TOKEN?.trim() || process.env.HAGISCRIPT_GITHUB_TOKEN?.trim()
  if (token) {
...
function getFixturePm2Entrypoint(runtimeRoot) {
  return process.platform === "win32"
    ? path.join(runtimeRoot, "program", "npm", "node_modules", "pm2", "bin", "pm2")
...
function isManagedPm2Service(serviceName) {
  return serviceName === "omniroute" || serviceName === "code-server" || serviceName === "server"
}
