...
const tempRoot = fs.mkdtempSync(
  path.join(os.tmpdir(), "hagiscript-runtime-management-")
);
...
try {
  ...
  await runStage("runtime state query", async () => {
    ...
    const nodeComponent = report.components.find(
      (item) => item.name === "node"
    );
    const dotnet = report.components.find((item) => item.name === "dotnet");
    const omniroute = report.components.find(
      (item) => item.name === "omniroute"
    );
    const codeServer = report.components.find(
      (item) => item.name === "code-server"
    );
    assert(
      report.layout?.separated === true,
      `Expected separated runtime layout. Output:\n${stdout}`
    );
    assertArrayEquals(
    ...
    assertPathsSeparated(
      ...
      "omniroute separation"
    );
    assertPathsSeparated(
      codeServer.programPaths,
      codeServer.externalDataPaths,
      "code-server separation"
    );
  ...
  await runStage("pm2 service lifecycle", async () => {
    ...
    npmSyncLines = [
      ...
      `- Manifest: ${pm2PackageManifestPath}`,
      "- Installed tooling through npm-sync: pm2@7.0.1"
    ];

    const pm2EnvOutput = await runCapture(
      process.execPath,
      [
        "dist/cli.js",
        "pm2",
    ...
    pm2EnvironmentLines = [
      "- Command: `hagiscript pm2 omniroute env --from-manifest <integration-manifest> --runtime-root <managed-root>`",
      "```text",
      ...pm2EnvOutput.trimEnd().split(/\r?\n/u),
      "```"
    ];
  ...
  await runStage("runtime remove purge", async () => {
    ...
    if (
      ...
      throw new Error(
        "Expected purge removal to clean code-server config directory."
      );
    }
  });
  ...
  await runStage("runtime partial failure", async () => {
    ...
    if (!alpha || alpha.status !== "installed") {
      throw new Error(
        `Expected alpha to remain installed after failure. Output:\n${stdout}`
      );
    }

    if (!failer || failer.status !== "failed") {
      throw new Error(
        `Expected failer to be marked failed. Output:\n${stdout}`
      );
    }

    if (!late || late.status !== "not-installed") {
      throw new Error(
        `Expected late to remain not-installed. Output:\n${stdout}`
      );
    }
  });
...
async function waitForManagedPm2Status(service, expectedStatus, options) {
  const attempts =
    process.platform === "darwin" ? 6 : process.platform === "win32" ? 6 : 3;
  let lastOutput = "";
...
function assertPathsSeparated(programPaths, externalDataPaths, label) {
  const overlaps = programPaths.filter((programPath) =>
    externalDataPaths.includes(programPath)
  );
  if (overlaps.length > 0) {
    throw new Error(
      `Expected ${label} to keep program/data paths separated, overlaps: ${overlaps.join(", ")}`
    );
  }
}
...
async function runStage(name, action) {
  ...
  try {
    ...
    log(
      `Stage failed: ${name} - ${error instanceof Error ? error.message : String(error)}`
    );
    throw error;
  }
}
